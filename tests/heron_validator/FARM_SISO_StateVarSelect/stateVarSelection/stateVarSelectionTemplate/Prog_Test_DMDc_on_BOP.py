...
fileFolder = os.path.join(os. getcwd(),'simulatedDataBOP')
...
U_Names = ["BOP_Demand_MW"]
Y_Names = [
    "Electric_Power", "Turbine_Pressure"
    ]
plot_profile = 2
...
for profile in range(1,3):
    ...
    if profile == 1:
        # 1: 1800s - 18000s
        test_variables = ['BOP.steamTurbine.sat_in.psat'];
    if profile == 2:
        # 2: 1800s - 5400s
        test_variables = ['BOP.steamTurbine.portHP.p', 'BOP.powerSensor.power', 'sinkElec.port.W'];
