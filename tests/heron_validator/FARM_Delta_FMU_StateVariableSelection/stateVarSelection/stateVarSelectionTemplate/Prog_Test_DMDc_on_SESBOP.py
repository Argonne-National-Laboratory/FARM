...
fileFolder = os.path.join(os. getcwd(),'simulatedData')
...
U_Names = ["SES_Demand_MW","BOP_Demand_MW"]
Y_Names = [
    "SES_Electric_Power", "SES_Firing_Temperature", "BOP_Electric_Power", "BOP_Turbine_Pressure"
    ]
plot_profile = 2
...
for profile in range(1,3):
    ...
    if profile == 1:
        # 1: 1800s - 18000s
        test_variables = ['BOP.generator1.Q_mech', 'BOP.generator1.Q_elec', 'BOP.sensorW.var', 'BOP.powerSensor.power', 'SES.generator.P_flow'];
    if profile == 2:
        # 2: 1800s - 5400s
        test_variables = ['BOP.CS.sensorBus.W_total', 'SES.CS.feedback_W_gen.u2', 'SES.ED.sensorBus.W_gen', 'SES.generator.P_flow', 'SES.powerGen.y'];
