...
vxSummary=[]; vxSummary_title = {'t', 'v0', 'v1', 'x0', 'x1'};
...
while ischar(tline)
    ...
    elseif startsWith(tline, "HaoyuDispatchSummary,")
        datadp = nan(1,numel(DispatchSummary_title)); 
        datavx = nan(1,numel(vxSummary_title)); 
        c = strsplit(tline,',');
        for i=1:numel(DispatchSummary_title)
            idx_c = find(strcmp(c, DispatchSummary_title{i}));
            datadp(i) = str2double(c{idx_c+1});
        end
        DispatchSummary=[DispatchSummary;datadp];        
        
        for i=1:numel(vxSummary_title)
            idx_c = find(strcmp(c, vxSummary_title{i}));
            datavx(i) = str2double(c{idx_c+1});
        end
