...
Num_x = 5; % SES.GTunit.combChamber.fluegas.h, BOP.CS.PID_TCV_opening.addP.u2
...
for i=(2+Num_r):2:(1+Num_r+3*Num_y)
...
for row_idx=1:6
    if any(t_learn)
        ...
        elseif row_idx==2 % output, y0, SES Power
            ...
            y_lb = min(DispatchSummary(:,8)); y_ub = max(DispatchSummary(:,12));
            legend('Upper Bound','Output #0','Lower Bound','Location','east','FontSize',FontSize)
        elseif row_idx==3 % output, y1, SES Temperature
            plot(time(t_learn),DispatchSummary(t_learn,13),'--r','LineWidth',3)
            plot(time(t_learn),DispatchSummary(t_learn,5),'-k')
            plot(time(t_learn),DispatchSummary(t_learn,9),'--b','LineWidth',3)
            ylabel('y1, SES Temp.(K)','FontSize',FontSize);
            y_lb = min(DispatchSummary(:,9)); y_ub = max(DispatchSummary(:,13));
            legend('Upper Bound','Output #1','Lower Bound','Location','east','FontSize',FontSize)
        ...
        elseif row_idx==5 % output, y2, BOP Power
            plot(time(t_learn),DispatchSummary(t_learn,14),'--r','LineWidth',3)
            plot(time(t_learn),DispatchSummary(t_learn,6),'-k')
            plot(time(t_learn),DispatchSummary(t_learn,10),'--b','LineWidth',3)
            ylabel('y2, BOP Pwr(MW)','FontSize',FontSize);
            y_lb = min(DispatchSummary(:,10)); y_ub = max(DispatchSummary(:,14));
            legend('Upper Bound','Output #2','Lower Bound','Location','east','FontSize',FontSize)
        elseif row_idx==6 % output, y3, BOP pressure
            ...
            y_lb = min(DispatchSummary(:,11)); y_ub = max(DispatchSummary(:,15));
            legend('Upper Bound','Output #3','Lower Bound','Location','east','FontSize',FontSize)
    ...
    elseif row_idx==2 % output, y0, SES Power
        ...
        legend('Upper Bound','Output #0','Lower Bound','Location','east','FontSize',FontSize)
    elseif row_idx==3 % output, y1, SES Temperature
        plot(time(t_dispa),DispatchSummary(t_dispa,13),'--r','LineWidth',3)
        plot(time(t_dispa),DispatchSummary(t_dispa,5),'-k')
        plot(time(t_dispa),DispatchSummary(t_dispa,9),'--b','LineWidth',3)
        ylabel('y1, SES Temp.(K)','FontSize',FontSize);
        y_lb = min(DispatchSummary(:,9)); y_ub = max(DispatchSummary(:,13));
        legend('Upper Bound','Output #1','Lower Bound','Location','east','FontSize',FontSize)
    elseif row_idx==4 % power setpoint v1, BOP
        ...
        y_lb = min(DispatchSummary(:,3)); y_ub = max(DispatchSummary(:,3));
    elseif row_idx==5 % output, y2, BOP Power
        plot(time(t_dispa),DispatchSummary(t_dispa,14),'--r','LineWidth',3)
        plot(time(t_dispa),DispatchSummary(t_dispa,6),'-k')
        plot(time(t_dispa),DispatchSummary(t_dispa,10),'--b','LineWidth',3)
        ylabel('y2, BOP Pwr(MW)','FontSize',FontSize);
        y_lb = min(DispatchSummary(:,10)); y_ub = max(DispatchSummary(:,14));
        legend('Upper Bound','Output #2','Lower Bound','Location','east','FontSize',FontSize)
    elseif row_idx==6 % output, y3, BOP pressure
